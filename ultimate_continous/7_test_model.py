...
if __name__ == "__main__":
    ...
    while not terminated:
        ...
        mean = policy(state_tensor) #extract mean from file
        normal_dist = Normal(mean, STD) # define a gaussian distribution
        action = normal_dist.sample().item() # sample an action from the distribution
        action_clamped = torch.clamp(torch.tensor(action), -1.0, 1.0) # clamp the action so that its in the rangr from -1 to 1
